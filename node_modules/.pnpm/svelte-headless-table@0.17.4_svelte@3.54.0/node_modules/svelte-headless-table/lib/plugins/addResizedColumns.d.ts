import type { NewTableAttributeSet, NewTablePropSet, TablePlugin } from '../types/TablePlugin';
import { type Writable } from 'svelte/store';
export interface AddResizedColumnsConfig {
    onResizeEnd?: (ev: Event) => void;
}
export declare type ResizedColumnsState = {
    columnWidths: Writable<Record<string, number>>;
};
export declare type ResizedColumnsColumnOptions = {
    initialWidth?: number;
    minWidth?: number;
    maxWidth?: number;
    disable?: boolean;
};
export declare type ResizedColumnsPropSet = NewTablePropSet<{
    'thead.tr.th': {
        (node: Element): void;
        drag: (node: Element) => void;
        disabled: boolean;
    };
}>;
export declare type ResizedColumnsAttributeSet = NewTableAttributeSet<{
    'thead.tr.th': {
        style?: {
            width: string;
            'min-width': string;
            'max-width': string;
            'box-sizing': 'border-box';
        };
    };
    'tbody.tr.td': {
        style?: {
            width: string;
            'min-width': string;
            'max-width': string;
            'box-sizing': 'border-box';
        };
    };
}>;
export declare const addResizedColumns: <Item>({ onResizeEnd }?: AddResizedColumnsConfig) => TablePlugin<Item, ResizedColumnsState, ResizedColumnsColumnOptions, ResizedColumnsPropSet, ResizedColumnsAttributeSet>;
